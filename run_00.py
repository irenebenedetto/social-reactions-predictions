...
if __name__ == "__main__":
    ...
    os.makedirs(f'{input_folder}/0.results', exist_ok=True)

    os.makedirs(f'{input_folder}/0.results/preprocessing', exist_ok=True)
    ...
    with open(f'{input_folder}/0.results/preprocessing/feature_list.json', 'w') as f:
        json.dump(vectorizer.get_feature_names(), f)
    with open(f'{input_folder}/0.results/preprocessing/tfidf_vectorizer.pkl', 'wb') as f:
    ...
    for model_name, model, param in zip(model_names, models, parameters):
        os.makedirs(f'{input_folder}/0.results/{model_name}', exist_ok=True)
        ...
        for y_col in y_cols:
            ...
            os.makedirs(f'{input_folder}/0.results/{model_name}/{y_col}', exist_ok=True)
            ...
            with open(f"{input_folder}/0.results/{model_name}/{y_col}/best_estimator.pkl", 'wb') as f:
                pkl.dump(best_predictor, f)

            with open(f"{input_folder}/0.results/{model_name}/{y_col}/best_scores_val.json", 'w') as f:
                json.dump(best_scores_val, f)

            with open(f"{input_folder}/0.results/{model_name}/{y_col}/best_params.pkl", 'wb') as f:
            ...
            print()
            with open(f"{input_folder}/0.results/{model_name}/{y_col}/test_scores.json", 'w') as f:
        ...
        #ys_test.reset_index(inplace=False, drop=True)
        output = pd.concat([output,  ys_test.reset_index(inplace=False, drop=True)], axis = 1)

        output.to_csv(f'{input_folder}/0.results/{model_name}/test_predictions.csv', index=None)

...
for model_name, model, param in zip(model_names, models, parameters):
    os.makedirs(f'./1.results/{model_name}', exist_ok=True)
    print(f'Predicting {model_name}')

    for y_col in y_cols:
    
        os.makedirs(f'./1.results/{model_name}/{y_col}', exist_ok=True)
        ...
        if SAVE:
            with open(f"./1.results/{model_name}/{y_col}/best_estimator.pkl", 'wb') as f:
                pkl.dump(best_predictor, f)

            with open(f"./1.results/{model_name}/{y_col}/best_scores_val.json", 'w') as f:
                json.dump(best_scores_val, f)

            with open(f"./1.results/{model_name}/{y_col}/best_params.pkl", 'wb') as f:
        ...
        print()
        with open(f"./1.results/{model_name}/{y_col}/test_scores.json", 'w') as f:
    ...
    output.to_csv(f'./1.results/{model_name}/test_predictions.csv')
